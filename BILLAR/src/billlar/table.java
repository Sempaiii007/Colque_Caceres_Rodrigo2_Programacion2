...
public class table extends JPanel implements Runnable {
    ...
    public table(LinkedList<boule> b) {
        clic = false;
        x0 = 0;
        y0 = -35;
        this.b = b;
        ...
        URL imageURL = getClass().getResource("/billlar/table.png");
        if (imageURL != null) {
            ee.setIcon(new ImageIcon(imageURL));
        } else {
    ...
    public void paint(Graphics g) {
        ...
        if (joueur == false && !bouleBouge(b)) {
            if (b.get(5).estRentree || (nbFauteJoueur2 >= 3)) {
                gkd.drawString("jugador 1 gana", 100, 100);
                gagne = true;
            } else {
                gkd.drawString("jugador 1 juega", 100, 100);
            }

        }
        if (joueur == true && !bouleBouge(b)) {
            if (b.get(9).estRentree || (nbFauteJoueur1 >= 3)) {
                gkd.drawString("jugador 2 gana", 100, 100);
                gagne = true;
            } else {
                gkd.drawString("jugador 2 juega", 100, 100);
            }
        }
        gkd.drawString("jugador 1 : " + nbFauteJoueur1 + " missed(s)", 100, 120);
        gkd.drawString("jugador 2 : " + nbFauteJoueur2 + " missed(s)", 100, 140);
    }
    ...
    public void run() {
        ...
        while (!gagne) {
            ...
            for (int i = 0; i < b.size(); i++) {
                ...
                b.get(i).pos.x += b.get(i).v.x * t0 / 1000;
                b.get(i).pos.y += b.get(i).v.y * t0 / 1000;

                vx0[i] = b.get(i).v;
                if (b.get(i).v.norme != 0) {
    ...
    public static void collision(LinkedList<boule> b, boolean[][] liste) {
        for (int i = 0; i < b.size(); i++) {
